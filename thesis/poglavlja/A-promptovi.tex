...
Ovaj dodatak sadrži materijale koji podržavaju transparentnost i reproducibilnost sistema EduGrader: šablone promptova korišćene u eksperimentima i nekoliko reprezentativnih primera ocenjivanja koje je sistem proizveo.
Promptovi su preneti verbatim iz konfiguracionih datoteka sistema, uključujući izvorne nedoslednosti u dijakritici, koje su zadržane jer su modeli primili upravo takav tekst.

\medskip
\noindent\textbf{Napomena.} Model ocenjuje na skali 0--100, a ocene se u analizi i u primerima ispod preslikavaju na skalu 0--10, radi poređenja sa ocenama nastavnika.

\subsection*{Uputstvo o formatu izlaza}
...
\subsection*{Promptovi strogosti za PRA režim}

U PRA režimu prompt uz sistemsku instrukciju sadrži pitanje, tačan odgovor nastavnika i studentski odgovor, svaki u zasebnom bloku.
...
Neutralna konfiguracija je namerno najkraća i ne uvodi dodatna pravila; blaga i stroga usmeravaju pažnju modela na poklapanja odnosno na nepoklapanja, a stroga uz to zadaje i eksplicitnu meru kazne.

\subsection*{Prompt za generisanje referentnog odgovora (GRA režim)}

U prvom koraku GRA režima model iz nastavnog materijala konstruiše referentni odgovor, koristeći sledeću instrukciju.
Ovaj korak ne zavisi od konfiguracije strogosti; ona se primenjuje tek u drugom koraku, pri ocenjivanju, i to istim promptovima kao u PRA režimu.
...
\subsection*{Promptovi za razmatrani treći režim}

Za varijantu sa nastavnim materijalom neposredno u promptu (odeljak~\ref{sec:rezim-udzbenik}) definisane su odgovarajuće instrukcije, koje se od PRA verzija razlikuju samo u opisu priloženog materijala.
Neutralna verzija navedena je kao ilustracija; budući da ovaj režim nije sproveden do kraja, njegovi rezultati se u radu ne koriste.
...
\subsection*{Primer A: sistem ocenjuje strože od nastavnika}
...
Nastavnik je pokušaj nagradio polovinom poena, dok je model, uz tačno izvedenu rekurentnu relaciju, veću težinu dao netačnom rezultatu.

\subsection*{Primer B: sistem ocenjuje blaže od nastavnika}
...
pristup n-tom elementu je laksi za niz, posto se elementi u nizu dodaju uz numeraciju, dok se kod listi pokazuje sa liste na listu bez znanja reda, sto bi znacilo da bi nam trebala petlja sa brojacem n dok ne dodjemo do n tog elemta liste
...
Student je tačno objasnio prvi deo: pristup n-tom elementu je efikasniji za niz nego za povezanu listu, jer se u listi mora prolaziti element po element (linearno), dok je u nizu direktan pristup.
Međutim, nije odgovorio na drugi deo pitanja (niz vs.\ dinamički niz).
Tačan odgovor za drugi deo je da je pristup n-tom elementu isti ($O(1)$) i za statički i za dinamički niz, jer oba koriste kontinuirani blok memorije i omogućavaju indeksirani pristup.

\end{tcolorbox}

Pitanje se sastoji od dva dela, a student je odgovorio samo na prvi.
Model je taj propust ispravno identifikovao i eksplicitno naveo tačan odgovor na izostavljeni deo, ali je odgovoru ipak dodelio nešto više od polovine poena, dok je nastavnik dodelio tačno polovinu.
Oba primera pokazuju da su obrazloženja sistema sadržajno tačna i proverljiva, čak i kada numerička ocena odstupa od nastavničke, što je i osnov za upotrebu sistema kao pomoći nastavniku, a ne kao njegove zamene.
