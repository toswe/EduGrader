...
\begin{itemize}
    ...
    izgrađena u tehnologijama \emph{React}~19 i \emph{TypeScript}, koju poslužuje \emph{Nginx} server.
    ...
    \item \textbf{Serverski sloj.} REST (engl. \emph{representational state transfer}) API izgrađen u tehnologijama \emph{Python}~3,
    \emph{Django}~5.1 i \emph{Django REST Framework}.
    ...
    \item \textbf{Sloj baze podataka.} Realizovan kroz \emph{Django} ORM (engl. \emph{object-relational mapping})
    nad bazom \emph{SQLite},
    sa entitetima za kurseve, testove, pitanja, studentske odgovore i ocene.
    \emph{SQLite} je izabran kao razvojna i prototipska baza, dovoljna za lokalno pokretanje:
    umesto zasebnog serverskog procesa, biblioteka \emph{SQLite} radi u samom procesu aplikacije,
    a celu bazu čuva u jednoj datoteci.
    \emph{Django} ORM dopušta prelazak na serverski sistem za upravljanje bazom (npr. \emph{PostgreSQL})
...
Slika prikazuje i evaluacioni postupak u sistemu DVC (engl. \emph{Data Version Control})\footnote{\url{https://dvc.org}},
...
\node[font=\scriptsize, anchor=south] at (fe.south) {\emph{React} 19 + \emph{TypeScript}};
...
\node[font=\scriptsize, anchor=north, yshift=-3mm] at (be.north) {\emph{Django} 5.1 + \emph{Django REST Framework}};
...
\node[font=\scriptsize] at ([yshift=-3mm]db.center) {\emph{SQLite}: kursevi, testovi, ocene};
...
\node[font=\small\bfseries, anchor=north] at (pipe.north) {DVC postupak};
...
Serverski sloj pristupa jezičkim modelima kroz jedinstven integracioni sloj (projektni obrazac fabrika)
koji apstrahuje API-je pojedinačnih provajdera.
Sistem za automatizovanu evaluaciju koristi isti integracioni sloj za grupne eksperimente.
...
Klijentska aplikacija je jednostranična aplikacija koja koristi biblioteku \emph{React} (verzija 19) i jezik \emph{TypeScript},
uz \emph{Vite} kao alat za izgradnju i razvojno okruženje.
Rutiranje je rešeno bibliotekom \emph{React Router}, kroz koju je aplikacija podeljena na dva modula sa odvojenim pravima pristupa:
...
Predlog modela je tako samo polazna tačka: nastavnik ga može prihvatiti ili prepraviti,
u skladu sa principom iz uvoda da je čovek sastavni deo sistema.

Elementi interfejsa su na engleskom, dok su pitanja, referentni odgovori i studentski odgovori na srpskom,
isto kao u svim eksperimentima u ovom radu.
...
Na snimku je nastavnik uneo konačnu ocenu 10, različitu od predloga modela,
što ilustruje da odluka ostaje na njemu.}
\label{fig:ui-ocena}
\end{figure}
...
a eksperimenti iz ovog rada pokreću je kroz evaluacioni postupak u sistemu DVC,
nad podacima pripremljenim u vidu CSV (engl. \emph{comma-separated values}) datoteka.
Izbor modela, režima ocenjivanja i konfiguracije strogosti zadaje se kroz parametarske datoteke tog postupka, a ne kroz veb interfejs.
...
Uloge (nastavnik ili student) ugrađene su u JWT i na osnovu njih se sprovodi kontrola pristupa rutama.
U razvojnom okruženju \emph{Vite} server prosleđuje API pozive ka serverskom delu,
dok se u produkciji statička verzija aplikacije poslužuje kroz \emph{Nginx}.

\section{Serverska aplikacija}

Serverski deo je \emph{Django} projekat podeljen na tri aplikacije, u skladu sa \emph{Django} konvencijom razdvajanja odgovornosti:
...
Sistem je zapakovan u \emph{Docker} kontejnere: jedan kontejner izvršava \emph{Django} aplikaciju,
a drugi \emph{Nginx} koji poslužuje klijentsku aplikaciju i prosleđuje API saobraćaj.
...
\begin{itemize}
    \item \textbf{OpenAI}, koristi zvanični OpenAI \emph{Python} SDK (engl. \emph{software development kit}).
    ...
    \item \textbf{Google Gemini}, koristi zvanični Google GenAI \emph{Python} SDK.
...
isti mehanizam koriste i veb aplikacija i sistem za automatizovanu evaluaciju.
