...
\begin{itemize}
    ...
    \item \textbf{Sloj baze podataka.} Realizovan kroz Django ORM nad sistemom SQLite, sa entitetima za kurseve,
...
Predlog modela je tako samo polazna tačka: nastavnik ga može prihvatiti ili prepraviti, što je princip čoveka u petlji iz uvoda.
...
a eksperimenti opisani u poglavlju~\ref{sec:pipeline} pokreću je kroz liniju obrade (engl. \emph{pipeline}) u sistemu DVC,
nad podacima pripremljenim u vidu CSV datoteka.
...
Autentifikacija koristi JWT, koji se čuva u \emph{session storage}-u pregledača;
...
Jezičkim modelima pristupa se isključivo preko API-ja provajdera (engl. \emph{provider}) u oblaku;
...
\newpage
\noindent Implementirane su dve integracije:
